...
export async function addHabit(userId, { name, icon, color, theme }) {
  ...
  const { data, error } = await supabase
    .from('habits')
    .insert({ user_id: userId, name, icon, color, theme: theme || 'default', logs: {}, notes: {} })
    .select()
    .single();
  if (error) {
...
export async function updateHabitFields(habitId, { name, icon, color, theme }) {
  const supabase = createClient();
  const { error } = await supabase.from('habits').update({ name, icon, color, theme }).eq('id', habitId);
