...
export const GAME_BALANCE = {
  ...
  MAX_LEVEL: 20,
  MAX_EXP: 840,
  // 8段階。数値を細かく刻むことで、頻繁に見た目が変わる楽しさを優先している。
  STAGE_LEVEL_THRESHOLDS: [0, 1, 4, 7, 10, 13, 16, 20],
};
...
export const STAGE_NAMES = ['たまご', '芽生え', 'よちよち期', '疾走期', '覚醒期', '飛翔期', '光輝期', '完全体'];
...
export function statsFromLogs(logs) {
  ...
  for (const date of dates) {
    ...
    if (level === 'none') {
      ...
      if (streak % GAME_BALANCE.STREAK_BONUS_EVERY === 0) exp += GAME_BALANCE.STREAK_BONUS_EXP;
    }
  }
...
export function seedDemoData() {
  ...
  const h1 = { id: genId(), name: '筋トレ', icon: '💪', color: '#22e2ff', createdAt: daysAgoStr(70), logs: {}, notes: {} };
  for (let i = 70; i >= 1; i--) h1.logs[daysAgoStr(i)] = weightedLevel([1, 1, 3, 6]);
  h1.notes[daysAgoStr(3)] = '今日はいつもより追い込めた。フォームも安定してきた気がする。';

  const h2 = { id: genId(), name: '英語学習', icon: '📚', color: '#8b5cf6', createdAt: daysAgoStr(20), logs: {}, notes: {} };
  for (let i = 20; i >= 1; i--) h2.logs[daysAgoStr(i)] = weightedLevel([2, 3, 3, 2]);

  const h3 = { id: genId(), name: '早起き', icon: '🌅', color: '#ff3d81', createdAt: daysAgoStr(15), logs: {}, notes: {} };
