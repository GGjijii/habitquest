...
function mapRow(row) {
  return {
    ...
    themeChangeCount: row.theme_change_count ?? 0,
...
export async function addHabit(userId, { name, icon, color, theme, frequencyDays }) {
  const supabase = createClient();
  const { data, error } = await supabase
    .from('habits')
    .insert({ user_id: userId, name, icon, color, theme: theme || 'default', theme_change_count: 0, frequency_days: frequencyDays || 1, logs: {}, notes: {} })
    .select()
    .single();
  if (error) {
...
export async function seedHabits(userId, habits) {
  ...
  const rows = habits.map((h) => ({
    ...
    theme_change_count: h.themeChangeCount || 0,
    frequency_days: h.frequencyDays || 1,
...
export async function updateHabitFields(habitId, { name, icon, color, theme, themeChangeCount, frequencyDays }) {
  const supabase = createClient();
  const { error } = await supabase.from('habits').update({ name, icon, color, theme, theme_change_count: themeChangeCount, frequency_days: frequencyDays || 1 }).eq('id', habitId);
