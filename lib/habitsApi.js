...
export async function addHabit(userId, { name, icon, color, theme }) {
  const supabase = createClient();
  const { data, error } = await supabase
    .from('habits')
    .insert({ user_id: userId, name, icon, color, theme: theme || 'default', theme_change_count: 0, logs: {}, notes: {} })
    .select()
    .single();
  if (error) {
...
export async function updateHabitFields(habitId, { name, icon, color, theme, themeChangeCount }) {
  const supabase = createClient();
  const { error } = await supabase.from('habits').update({ name, icon, color, theme, theme_change_count: themeChangeCount }).eq('id', habitId);
